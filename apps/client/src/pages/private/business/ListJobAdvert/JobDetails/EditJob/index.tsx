...
function EditJobComponent() {
  ...
  const { reset, register, watch, setValue, handleSubmit, formState: { isSubmitting } } = useForm({
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between max-h-60 mt-2 overflow-auto">
        <JoditEditor value={watch("description")} onChange={(val) => setValue("description", val)} />
      </div>
      <div className="flex justify-between mt-2">
        <div className="flex gap-2 items-center">
          <div className='w-4 h-4 rounded-[50%]' style={{ background: currentStatus.color }}></div>
          <Select onValueChange={(val) => setValue("status", val as StatusJob)}>
            <SelectTrigger>
